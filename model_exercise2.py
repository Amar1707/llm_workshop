...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        if config.get("freeze", False):
            # to-do: freeze the self.transformer parameters
    ...
    def forward(self, idx, targets=None, prompts=None, pad_mask=None):
        ...
        if hasattr(self, "c_head"):
            # to-do: generate logits by forwarding only the last timestep
            if targets is not None:
                # to-do: if we are given some desired targets also calculate the loss
            else:
                loss = None
        else:
            if targets is not None:
