...
task = "last_char"  # name of task in case of fine-tuning. used to understand type of task and read its dataset
prompt_vocab_size = 3  # prompt-tuning. Supports only one task
classification_task = "classification" in task

model_extended_config = dict(prompt_vocab_size = prompt_vocab_size)
...
OUT_CHECKPOINT = "last_char_prompt_tuning.pt"  # checkpoint to write the model to
