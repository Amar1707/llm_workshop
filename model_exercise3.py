...
class Adapter(nn.Module):
    def __init__(self, config):
        super().__init__()
        # to-do: fill in the in_features and out_features for both the linear layers
        # "adapter_size" parameter is available in config
...
class Block(nn.Module):
    ...
    def forward(self, x, pad_mask):
        ...
        if hasattr(self, "adapter"):
            # to-do: forward pass through the adapter, just like mlp (or attn)
        return x
